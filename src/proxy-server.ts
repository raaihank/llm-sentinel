...
import { ConfigManager, LLMSentinelConfig } from './config';
...
export class ProxyServer {
  ...
  private appConfig: LLMSentinelConfig;
  ...
  constructor(config: ProxyServerConfig) {
    ...
    this.logger = new Logger(
      this.appConfig.logging.logToConsole, 
      this.appConfig.logging.logToFile
    );
    this.setupProxies();
  }
  ...
  private onProxyReqHandler = (proxyReq: http.ClientRequest, req: any, _res: http.ServerResponse): void => {
    ...
    if (req.body) {
      ...
      try {
        ...
        let findings: Array<{entityType: string, masked: string, count: number}> = [];
  ...
  private onProxyResHandler = (proxyRes: http.IncomingMessage, req: any, _res: http.ServerResponse): void => {
  ...
  private notifySensitiveData(finding: string): void {
  ...
  private setupProxies(): void {
    ...
    this.app.use('/openai', createProxyMiddleware(openaiOptions));
    this.app.use('/ollama', createProxyMiddleware(ollamaOptions));
  }
