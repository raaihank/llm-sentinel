#!/usr/bin/env python3
"""
Gandalf Dataset Benchmark for LLM-Sentinel
Uses Lakera's Gandalf prompt injection dataset from HuggingFace

Dataset: lakera/gandalf_ignore_instructions
Contains real prompt injection attempts from the Gandalf game
"""
...
def print_results(results: pd.DataFrame, latency_stats: dict, model_name: str = "LLM-Sentinel"):
    ...
    # True positives (correctly blocked injections)
    tp = results.loc[('gandalf_injection', True), 'correct'] if ('gandalf_injection', True) in results.index else 0
    tp_total = results.loc[('gandalf_injection', True), 'total'] if ('gandalf_injection', True) in results.index else 0
...
def main():
    parser = argparse.ArgumentParser(description='Run Gandalf benchmark against LLM-Sentinel')
    ...
    # Load Gandalf dataset
    df = load_gandalf_dataset(args.max_injections)
    ...
    # Print results
    score = print_results(results, latency_stats)
    
    # Save results
    timestamp = pd.to_datetime('today').strftime('%Y%m%d_%H%M%S')
    results_file = f"gandalf_results_{timestamp}.csv"
